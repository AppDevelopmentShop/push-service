    ...
    it('should register token user1', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          res.body.device.should.include.keys(deviceModel)
          res.body.status.should.eql('success')
          res.body.device.token.should.eql('token')
          res.body.device.user_id.should.eql(1)
          res.body.device.id.should.eql(4)
          done()
        })
    })
    it('should register token again user2', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          res.body.status.should.eql('success')
          res.body.device.token.should.eql('token')
          res.body.device.user_id.should.eql(2)
          res.body.device.id.should.eql(5)
          done()
        })
    })
    it('should register token user3', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          res.body.device.should.include.keys(deviceModel)
          res.body.status.should.eql('success')
          res.body.device.token.should.eql('token1')
          res.body.device.user_id.should.eql(3)
          res.body.device.id.should.eql(6)
          done()
        })
    })
  })
